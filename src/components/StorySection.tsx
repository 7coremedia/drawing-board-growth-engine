...
const StorySection = () => {
  ...
  return <section className="py-32 bg-gradient-to-br from-gray-50 to-white relative overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="flex justify-center mb-16">
          <div className="w-full max-w-4xl">
            <div className="bg-gradient-to-br from-green-800 to-green-900 p-8 glow w-full h-64 border border-green-700 flex items-center justify-between rounded-3xl">
              {/* Notification circle */}
              ...
              <div className="flex-shrink-0 ml-8">
                <Button onClick={() => setIsLetterOpen(true)} className="bg-accent text-primary hover:bg-accent/90 font-black text-lg px-8 py-4 rounded-xl flex items-center gap-3">
                  Read Letter
                  <ArrowRight size={20} />
                </Button>
              </div>
        ...
        {isLetterOpen && <div className="animate-roll-down overflow-hidden">
            ...
            fontFamily: 'Satoshi, sans-serif'
              ...
              if (line === "Customers.") {
                return <p key={index} className={`font-black text-primary text-3xl lg:text-4xl transition-all duration-1000 transform ${shouldShow ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
                        {line}
                      </p>;
              }
              if (line.includes("Rule #1")) {
                return <div key={index} className={`bg-primary/10 p-8 rounded-xl glow transition-all duration-1000 transform ${shouldShow ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
                        <p className="font-black text-2xl lg:text-3xl text-primary">{line}</p>
                      </div>;
              }
              return <p key={index} className={`text-gray-700 transition-all duration-1000 transform ${shouldShow ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`} style={{
